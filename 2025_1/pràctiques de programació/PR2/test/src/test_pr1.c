...
bool run_pr1_ex2(tTestSection *test_section, const char *input) {
	...
	csv_parse(&test_people, test_people_str, "PERSON");
	...
	/////////////////////////////
	failed = false;
	start_test(test_section, "PR1_EX2_9", "Remove all companies from the catalog", 0.2);
	if (fail_all) {
		failed = true;
	} else {
...
bool run_pr1_ex3(tTestSection *test_section, const char *input) {
	...
	if (fail_all) {
		...
		if (nJobs != 1) {
			...
			// Valid job
			csv_initEntry(&entry);
			csv_parseEntry(&entry, "8;Computer scientist;1;07/08/2025;01/11/2025;16576;20000;D60487219", "JOB");
			...
			// Invalid job
			csv_initEntry(&entry);
			csv_parseEntry(&entry, "9;CTO;1;07/08/2025;01/11/2025;47000;59500;D60487219", "JOBB");
			api_addJob(&data, entry);
			csv_freeEntry(&entry);
			
			// Invalid job
			csv_initEntry(&entry);
			csv_parseEntry(&entry, "9;CTO;1;07/08/2025;01/11/2025;47000;59500", "JOB");
			api_addJob(&data, entry);
			csv_freeEntry(&entry);
			
			// Non-existing job
			csv_initEntry(&entry);
			csv_parseEntry(&entry, "9;CTO;1;07/08/2025;01/11/2025;47000;59500;NON-EXISTING", "JOB");
			api_addJob(&data, entry);
			csv_freeEntry(&entry);
