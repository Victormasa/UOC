...
/*
void LSim_sendCorreccio(bool es_correcte, char nota, char* sumari, char* resultat_detallat)
{
    // Nothing to do, this is a mock
}

void LSimLogger_log(char *nivell, char *missatge){
    // Nothing to do, this is a mock
}
*/
...
// Get test statistics
void testSuite_getStats(tTestSuite* object, int* total, int* passed, int* failed, int* not_implemented, unsigned short* mark) {
    int i;
    int s_total, s_passed, s_failed, s_not_implemented;
    unsigned short s_mark;
    assert(object != NULL);
    assert(total != NULL);
    assert(passed != NULL);
    assert(failed != NULL);
    ...
    *mark = 0;
    for (i = 0; i < object->numSections; i++) {
        testSection_getStats(&(object->sections[i]), &s_total, &s_passed, &s_failed, &s_not_implemented, &s_mark);
        *total += s_total;
        *passed += s_passed;
        *failed += s_failed;
...
void testSuite_print(tTestSuite* object, tAppArguments parameters) {
    ...
    testSuite_getStats(object, &total, &passed, &failed, &not_implemented, &mark);
...
#ifdef DSLAB
    char summary[100];
    sprintf(summary, "\%d/%d [%0.2f%%]. Mark: %d\n", passed, total, percent, mark);
...
void testSuite_export(tTestSuite* object, const char* output) {
    ...
    fprintf(fout, " \"total\": %d, \"passed\": %d, \"failed\": %d, \"not_implemented\": %d, \"mark\": %d, \"sections\": [",
        total, passed, failed, not_implemented, mark);
...
void testSection_free(tTestSection* object) {
    ...
    if (object->exercises != NULL) {
        for (i = 0; i < object->numExercises; i++) {
            exercise_free(&(object->exercises[i]));
        }
        free(object->exercises);
    }
}
...
void testSection_addTest(tTestSection* object, const char* code, const char* description, tTestResult result) {
    ...
    // Add an exercise if doesn't exists
    ex_code = testExercise_getCode(code);
    assert(ex_code != NULL);
    exercise = testSection_addExercise(object, ex_code, 0);
    assert(exercise != NULL);

    // Add test to exercise
    testExercise_addTest(exercise, code, description, result);
    free(ex_code);
}
...
tTest* testSection_getTest(tTestSection* object, const char* test_code) {
    ...
    assert(test_code != NULL);
    for (i = 0; i < object->numExercises; i++) {
        for (j = 0; j < object->exercises[i].numTests; j++) {
            if (strcmp(object->exercises[i].tests[j].code, test_code) == 0) {
                return &(object->exercises[i].tests[j]);
            }
        }
    }
    return NULL;
...
// Get test statistics
void testSection_getStats(tTestSection* object, int* total, int* passed, int* failed, int* not_implemented, unsigned short* mark) {
    int i, j, passed_ex;
    assert(object != NULL);
    assert(total != NULL);
    assert(passed != NULL);
    ...
    *mark = 0;
    for (i = 0; i < object->numExercises; i++) {
        *total += object->exercises[i].numTests;
        passed_ex = 0;
        for (j = 0; j < object->exercises[i].numTests; j++) {
            if (object->exercises[i].tests[j].result == TEST_PASSED) {
                passed_ex++;
            }
            else if (object->exercises[i].tests[j].result == TEST_FAILED) {
                (*failed)++;
            }
            else if (object->exercises[i].tests[j].result == TEST_NOT_IMPLEMENTED) {
                (*not_implemented)++;
            }
        }
        // Calculate mark/grade
        *passed += passed_ex;
        *mark += round((((float)passed_ex) / object->exercises[i].numTests) * object->exercises[i].mark);
    }
}
...
void testSection_print(tTestSection* object, tBuffer* buffer) {
    ...
    testSection_getStats(object, &total, &passed, &failed, &not_implemented, &mark);

    bprintf(buffer, "\n\t=================================================================\n");
    bprintf(buffer, "\t%s\n", object->title);
    bprintf(buffer, "\t=================================================================\n");
    if (object->numExercises == 0) {
        bprintf(buffer, "\tNO TEST DEFINED\n");
    }
    else {
        for (i = 0; i < object->numExercises; i++) {
            ...
            for (j = 0; j < object->exercises[i].numTests; j++) {
                test_print(&(object->exercises[i].tests[j]), buffer);
            }
        }
    }
    bprintf(buffer, "\t=================================================================\n");
...
void testSection_export(tTestSection* object, FILE* fout) {
    ...
    fprintf(fout, "{ \"code\": \"%s\", \"title\": \"%s\", \"total\": %d, \"passed\": %d, \"failed\": %d, \"not_implemented\": %d, \"mark\": %d, \"tests\": [",
        object->code, object->title, total, passed, failed, not_implemented, mark);
    ...
    for (i = 0; i < object->numExercises; i++) {
        for (j = 0; j < object->exercises[i].numTests; j++) {
            ...
            test_export(&(object->exercises[i].tests[j]), fout);
        }
    }
...
// If not exists, add an exercise to a Section
tExercise* testSection_addExercise(tTestSection* object, const char* code, unsigned short mark) {
    tExercise* exercise;
    assert(object != NULL);

    exercise = testExercise_getExercise(object, code);
    if (exercise == NULL) {
        object->numExercises++;
        if (object->exercises == NULL) {
            object->exercises = (tExercise*)malloc(object->numExercises * sizeof(tExercise));
        }
        else {
            object->exercises = (tExercise*)realloc(object->exercises, object->numExercises * sizeof(tExercise));
        }
        assert(object->exercises != NULL);
        exercise = &(object->exercises[object->numExercises - 1]);
        exercise_init(exercise, code, mark);
    }
    return exercise;
}

// Add a test to an exercise
void testExercise_addTest(tExercise* object, const char* code, const char* description, tTestResult result) {
    assert(object != NULL);
...
// Get a pointer to an exercise
tExercise* testExercise_getExercise(tTestSection* object, const char* code) {
    int i = 0;
    while (i < object->numExercises) {
        if (strcmp(object->exercises[i].code, code) == 0)
            return &(object->exercises[i]);
        i++;
    }
    return NULL;
}

// Get the code associated to an exercise from test code
char* testExercise_getCode(const char* test_code) {
    char* ex_code;

    // Handle NULL or empty test_code
    if (test_code == NULL || *test_code == '\0') {
        return NULL;
    }

    // Look for underscore
    const char* last_underscore = strrchr(test_code, '_');
    if (last_underscore == NULL || *(last_underscore + 1) == '\0') {
        return NULL;
    }

    // Copy the prefix into the new string
    size_t prefix_len = last_underscore - test_code;
    if (prefix_len > 0) {
        ex_code = (char*)malloc(prefix_len + 1);
        if (ex_code != NULL) {
            strncpy(ex_code, test_code, prefix_len);
            ex_code[prefix_len] = '\0'; // Ensure null termination
            return ex_code;
        }
    }
    return NULL;
...
void exercise_init(tExercise* exercise, const char* code, unsigned short mark) {
    exercise->numTests = 0;
    exercise->mark = mark;
    exercise->code = (char*)malloc((strlen(code) + 1) * sizeof(char));
    strcpy(exercise->code, code);
    exercise->tests = NULL;
}

// Remove an exercise
void exercise_free(tExercise* object) {
    int i;
    assert(object != NULL);
    assert(object->code != NULL);
    assert(object->tests != NULL);
